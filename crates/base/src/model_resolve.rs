...
//!
//! The resolver is intentionally pure (no I/O, no locks) so callers can
//! compose it with their own state ownership. Both `create_session` and
//! `set_model` flow through it.
//!
//! ## Resolution order
//!
//! 1. **Operator alias map** (if provided): if `raw` matches a key here, the
//!    target is taken from the operator map.
...
//! (optionally `provider/model-id`), never another alias. This makes cycles
...
//!
//! Aliases targets may be prefixed with a provider name and a `/`. We split
//! on the **first** `/` only, so `"foo/bar/baz"` is parsed as
//! `provider = "foo"`, `id = "bar/baz"` — letting unusually-shaped model ids
//! that contain slashes still resolve.
//!
//! When the caller provides an explicit `provider_filter` (the `provider`
//! field from a `CreateSession`/`SetModel` request), it always takes
//! precedence over a `provider/` prefix in the alias target.
...
pub enum ResolveError {
    /// `raw` matched an alias but its target does not point to any known
    /// model. Always an error: surfaces config bugs to the user instead of
    ...
    UnknownAlias {
        ...
        target: String,
        /// Where the alias came from: `"operator"` or `"global"`.
        scope: &'static str,
    },
    /// `raw` was not an alias and not a known model id. Callers may choose
    /// to surface this as an error or fall back to a default.
...
pub fn resolve_model<'a>(
    ...
    operator_aliases: Option<&HashMap<String, String>>,
    global_aliases: &HashMap<String, String>,
    all_models: &'a [Model],
) -> Result<&'a Model, ResolveError> {
    // 1+2. Alias lookup (operator takes precedence over global). An empty
    // operator map must not shadow the global lookup, so we treat it as None.
    let (target, alias_scope): (&str, Option<&'static str>) = if let Some(map) =
        operator_aliases.and_then(|m| if m.is_empty() { None } else { Some(m) })
        && let Some(t) = map.get(raw)
    {
        (t.as_str(), Some("operator"))
    ...
    // 3. Parse `provider/id` form. Split on the FIRST `/` only so model ids
    //    containing slashes are preserved in the id half.
...
mod tests {
    use super::*;
    use crate::types::{ModelCost, ThinkingStyle};
    ...
    #[test]
    fn operator_alias_hit() {
        let models = make_models();
        let global = HashMap::new();
        let mut operator = HashMap::new();
        operator.insert("smart".into(), "haiku-4".into());
        let m = resolve_model("smart", None, Some(&operator), &global, &models).unwrap();
        assert_eq!(m.id, "haiku-4");
    }

    #[test]
    fn operator_overrides_global() {
        let models = make_models();
    ...
    #[test]
    fn unknown_operator_alias_target_reports_operator_scope() {
        let models = make_models();
        let global = HashMap::new();
        let mut operator = HashMap::new();
        operator.insert("planner".into(), "ghost".into());
        let err = resolve_model("planner", None, Some(&operator), &global, &models).unwrap_err();
        assert!(matches!(
            err,
            ResolveError::UnknownAlias {
                ref name,
    ...
    #[test]
    fn empty_operator_map_falls_through_to_global() {
        let models = make_models();
        let mut global = HashMap::new();
        global.insert("smart".into(), "opus-4".into());
        let operator = HashMap::new();
        let m = resolve_model("smart", None, Some(&operator), &global, &models).unwrap();
        assert_eq!(m.id, "opus-4");
    }
}
