//! Plugin SDK for tars.
//!
//! Defines the tool execution contract (`ToolExecutor`) that the server uses to
...
///
/// Single canonical source — both the engine's `system_prompt::build` and the
/// worker crate delegate here.
pub fn default_tool_prompts() -> Vec<ToolPrompt> {
    vec![
        ...
        ToolPrompt {
            ...
            snippet: "Run lint/syntax diagnostics on specific files and get structured per-file feedback (built-in: Rust via cargo check; configurable via .tars/diagnostics.toml)".into(),
            guidelines: vec![
